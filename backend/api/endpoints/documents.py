...
def upload_document(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    if ext not in SUPPORTED_FILE_TYPES:
        raise HTTPException(status_code=400, detail=f"Supported types: {SUPPORTED_FILE_TYPES}")

    # Read content to check size, then reset pointer
    content = file.file.read()
    if len(content) > settings.MAX_FILE_SIZE:
        raise HTTPException(status_code=400, detail=f"Max size is {settings.MAX_FILE_SIZE} bytes")
    file.file.seek(0)
